...
#then retrieves the hidden message and compares with original.
#Finally, it displays the first 60 chars of the retrieved message.
#
#If -v option specified, then display commands before execution.

if [ $# -lt 3  ] || [ $1 != '-v' -a $# -gt 3 ] || [ $1 == '-v' -a $# -gt 4 ]
then
...
$VERBOSE && echo $cmd
eval $cmd
[ $? -eq 0 ] || { echo "image comparison failed" && exit 1; }

#setup for hide
data=`ruby -ne \
  'require "json"; puts JSON.generate({outGroup: "outputs", msg: $_})' \
  $MSG_PATH`
...
#unhide
cmd="curl --silent --write-out '%{http_code}' --output '$JSON' '$location'"
$VERBOSE && echo $cmd
status=`eval $cmd`
...
ruby -ne 'require "json"; print JSON.parse($_)["msg"]' $JSON >$MSG

#compare extracted msg with original
cmd="cmp '$MSG_PATH' '$MSG'"
$VERBOSE && echo $cmd
eval $cmd
[ $? -eq 0 ] || { echo "message comparison failed" && exit 1; }
